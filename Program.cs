...
namespace c__scratchpad
{
  ...
  class NumbersCalculator
  {
    ...
    private static void GetCounter(NumbersCalculator newNumbersCalc)
    {
      bool display = true;
      string userInput = "";
      ...
      while (display)
      {
        Console.Write("How many numbers do you want to {0}? ", newNumbersCalc.Operation);
        userInput = Console.ReadLine();
        if (Program.CheckNumber(0, userInput) > 0) 
        {
          display = false;
          newNumbersCalc.Counter = int.Parse(userInput);
        }
